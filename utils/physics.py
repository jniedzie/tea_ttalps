...
def Gammaatohad(ma, coeffs, Lambda):
  ...
  try:
    ...
    pass
  gamma = abs(cGGeff)**2 * alphaS**2 * ma**3 * (1 + (95/4-7*nQ/6) *
                                                alphaS/math.pi) * 2/(math.pi * Lambda**2)  # *(16*math.pi**2)
...
def g00(r):
  ...
  def integrand(z): return math.sqrt(1-4*r/z) * math.sqrt(1+z**2+r**2-2*r-2*z-2*z*r)
  lim_0 = 4*r
...
def gpm(r):
  ...
  def integrand(z): return math.sqrt(1-4*r/z) * math.sqrt(1+z**2+r**2-2*r-2*z-2*z*r) * (z-r)**2
  lim_0 = 4*r
  lim_1 = (1-math.sqrt(r))**2
  integral = scipy.integrate.quad(integrand, lim_0, lim_1)
  factor = 12/(1-r)**2

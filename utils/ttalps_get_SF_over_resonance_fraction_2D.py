...
skims = {
    ...
    "_PatDSA": ("skimmed_looseSemimuonic_v3_SR", "JPsiDimuonsPatDSA", "noDimuonEffSFs_noMatching_ABCD"),
...
# output_graph_dir = "../ttalps_get_SF_over_resonance_fraction_graphs"
# if len(years) > 1:
#     output_graph_dir = None
...
resonance_variables_ = {
    ...
    "_PatDSA": [
        "eta_irr2",
        "eta_irr2_logDRlt-1_logDxySig2gt0",
        "eta_irr2_logDRlt-0p8_logDxySig2gt0p2",
        "eta_irr2_logDRlt-1p2_logDxySig2gt-0p2",
        "eta_irr2_logDRgt-1_logDxySig2lt0",
        "eta_irr2_logDRgt-1p5_logDxySig2lt0p5",
        "eta_irr2_logDRgt-2_logDxySig2lt1",

        "eta_irr2_logDRgt-1p2_logDxySig2lt-0p2",
        "eta_irr2_logDRlt-0p5_logDxySig2lt-0p2",
        "eta_irr2_logDRgt-1_logDxySig2gt0",
        "eta_irr2_logDRgt-2_logDxySig2gt0",
...
sf_bin_variable = "pt_irr2"
# sf_bin_variable = "pt_irr3"
...
def plot_sf_with_variations(fit_results, x_min, x_max, n_points=200, canvas_name="c_sf_variations"):
    ...
    for i, pt in enumerate(sorted(fit_results.keys())):
        ...
        if first:
            ...
            gr_c.GetYaxis().SetTitle("Scale factor")
...
def draw_sf_points(resonant_fractions, scale_factors, pt_bin, marker_style=20, color=ROOT.kBlack):
    """
    ...
    for variable in resonant_fractions.keys():
        ...
        sf_dict = list(scale_factors[variable].values())[0]
        SF, SF_up, SF_down = sf_dict[pt_bin]
...
def main():
    ...
    for variable in resonance_variables:
        ...
        hist_name = f"{dimuon_collection}_{sf_bin_variable}_vs_{variable}{category}"
        if reverted_matching:
            hist_name = f"{dimuon_collection}_revertedMatching_{sf_bin_variable}_vs_{variable}{category}"
        ...
        data_histogram = sfProducer.getDataHistogram2D(histogram2D)
        background_histogram = sfProducer.getBackgroundHistogram2D(histogram2D)
        data_hist = data_histogram.hist
        background_hist = background_histogram.hist
        scale_factors[variable] = sfProducer.getDataMCRatios2D(data_histogram, background_histogram)
        ratio_hist[variable],ratio_unc_hist_up[variable],ratio_unc_hist_down[variable]  = sfProducer.getDataMCRatioHists2D(data_histogram, background_histogram)

        
        pt_bins_ = list(scale_factors[variable][list(scale_factors[variable].keys())[0]].keys())
        resonant_hist_name = f"{dimuon_collection}{resonant_str}_{sf_bin_variable}_vs_{variable}{category}"
        if reverted_matching:
            resonant_hist_name = f"{dimuon_collection}{resonant_str}_revertedMatching_{sf_bin_variable}_vs_{variable}{category}"
        nonresonant_hist_name = f"{dimuon_collection}Non{resonant_str}_{sf_bin_variable}_vs_{variable}{category}"
        if reverted_matching:
            nonresonant_hist_name = f"{dimuon_collection}Non{resonant_str}_revertedMatching_{sf_bin_variable}_vs_{variable}{category}"

        resonant_histogram2D = Histogram2D(
        ...
        nonresonant_histogram2D = Histogram2D(
        ...
        resonant_background_histogram = sfProducer.getBackgroundHistogram2D(resonant_histogram2D)
        nonresonant_background_histogram = sfProducer.getBackgroundHistogram2D(nonresonant_histogram2D)
        for i in range(1, background_hist.GetNbinsY()+1):
            resonant_events[variable][pt_bins_[i-1]] = resonant_background_histogram.hist.GetBinContent(1,i)
            nonresonant_events[variable][pt_bins_[i-1]] = nonresonant_background_histogram.hist.GetBinContent(1,i)
            n_tot_events[variable][pt_bins_[i-1]] = resonant_background_histogram.hist.GetBinContent(1,i) + nonresonant_background_histogram.hist.GetBinContent(1,i)
            resonant_fractions[variable][pt_bins_[i-1]] = resonant_background_histogram.hist.GetBinContent(1,i)/n_tot_events[variable][pt_bins_[i-1]] if n_tot_events[variable][pt_bins_[i-1]] != 0.0 else 0.0

    variable0 = resonance_variables[0]
    pt_bins = scale_factors[variable0][list(scale_factors[variable0].keys())[0]].keys()
    fit_results = {}
    ...
    for i, pt_bin in enumerate(pt_bins):
        gr_points = draw_sf_points(resonant_fractions, scale_factors, pt_bin, color=colors[i])
    ...
    c.SaveAs(f"{output_dir}/sfs_{sf_bin_variable}.pdf")

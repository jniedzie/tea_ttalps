...
# Histograms
hist_path = "histograms_muonSFs_muonTriggerSFs_pileupSFs_bTaggingSFs_PUjetIDSFs_JPsiDimuons_LooseNonLeadingMuonsVertexSegmentMatch"  # all SFs
...
def main():
  ...
  for ratio_name, ratio in matching_ratios.items():
    ...
    for category in ("", "Pat", "PatDSA", "DSA"):
      ...
      background_yield, background_uncertainty_down, background_uncertainty_up = sfProducer.getYieldAndAsymmetricUncertainties(
          background_stack.GetStack().Last())
      print(f"Data yields: {data_yield:.5f} pm {data_uncertainty_down:.5f} (down) pm {data_uncertainty_up:.5f} (up)")
      print(
          f"Background yields: {background_yield:.5f} pm {background_uncertainty_down:.5f} (down) pm {background_uncertainty_up:.5f} (up)")

      ratio, ratio_uncertainty_down, ratio_uncertainty_up = sfProducer.getDataMCRatio(
          data_yield, data_uncertainty_down, data_uncertainty_up, background_yield, background_uncertainty_down, background_uncertainty_up)
      ...
      background_yields[ratio_name, category] = [background_yield,
                                                 background_uncertainty_up, background_uncertainty_down]

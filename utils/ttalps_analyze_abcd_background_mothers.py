...
skim = ("skimmed_looseSemimuonic_v2_SR_segmentMatch1p5", "SRDimuonsDSAChi2DCADPhi", "LooseNonLeadingMuonsVertexSegmentMatch_genABCD")
hist_path = f"histograms_muonSFs_muonTriggerSFs_pileupSFs_bTaggingSFs_PUjetIDSFs_dimuonEffSFs_DSAEffSFs_jecSFs"
...
mother_categories = ["X","W","tau","B","D","d","u","s","c","b","t","e","mu","g","gamma","Z","rho","pi0","omega","K0","phi","upsilon","JPsi","other"]

colors = [ROOT.kGreen+2, ROOT.kRed, ROOT.kOrange+1, ROOT.kMagenta+1, ROOT.kPink+1, ROOT.kCyan+1, ROOT.kViolet+1, ROOT.kTeal+1, ROOT.kYellow+1, ROOT.kAzure+1, ROOT.kGreen+3, ROOT.kMagenta+3 ,ROOT.kRed+3, ROOT.kOrange+3, ROOT.kBlue+3, ROOT.kBlue-3, ROOT.kBlack]
mother_naming = {
...
def main():
  ...
  legend = ROOT.TLegend(legend_positions[category][0], legend_positions[category][1], legend_positions[category][2], legend_positions[category][3])
  legend.SetBorderSize(0)
  legend.SetFillStyle(0)
  legend.SetTextSize(0.03)
  legend2 = ROOT.TLegend(legend_positions[category][0], legend_positions[category][1], legend_positions[category][2], legend_positions[category][3])
  legend2.SetBorderSize(0)
  legend2.SetFillStyle(0)
  legend2.SetTextSize(0.03)
  legend3 = ROOT.TLegend(legend_positions[category][0], legend_positions[category][1], legend_positions[category][2], legend_positions[category][3])
  ...
  if do_mother_ids:
    n_events_per_resonance_motherid1_sorted = dict(sorted(n_events_per_resonance_motherid1.items(), key=lambda item: item[1], reverse=True))
    merged_resonance_motherid1_histograms = merge_histograms(resonance_motherid1_histograms,n_events_per_resonance_motherid1_sorted)

    legend3 = setup_resonance_legend(legend3, merged_resonance_motherid1_histograms, n_events_per_resonance_motherid1_sorted)
